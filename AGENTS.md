...
- Your IRC identity is `codex-cll-<item>`, pre-provisioned by the supervisor. The `[mcp_servers.irc]` MCP server in `~/.codex/config.toml` provides the tools; your identity is fixed at launch time by three `-c mcp_servers.irc.env.*` overrides on the `codex exec` invocation (`IRCV3_MCP_CONFIG_DIR`, `IRCV3_MCP_STATE_DIR`, `IRCV3_MCP_SECRET_BACKEND`, per `~/git/agent-ops/README.md`) — the prompt itself cannot change it. Your dispatch prompt states the role and the exact channel; if it names an IRC identity but the tools connect as `codex-default`, the dispatch is broken — say so in your report instead of posting under the wrong account.
