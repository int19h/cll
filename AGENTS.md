...
the reviewer to reread the changed passages. Before a long pause, durably send a
status/handoff with the issue/PR, report path, branch/worktree, exact HEAD,
sources and decisions already consulted, findings settled or still open, checks
completed or pending, blockers, and relevant message IDs.
